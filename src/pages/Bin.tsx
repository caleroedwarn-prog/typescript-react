...
function Bin() {
    const { task, setTask, deletedTask, setDeletedTask } = useOutletContext<ContextType>();
  ...
  return (
    <>
      ...
      {deletedTask.length === 0  && <p> No deleted tasks</p>}

      {deletedTask.map((task, index) => (
        <div key={index}>
            <h3> {task.name} </h3>
            <button type="button" onClick={() => restoreTask(index)}>Restore</button>
        </div>
      ))}
    </>
  );
